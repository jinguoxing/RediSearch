...
def testCursors(self):
    ...
    resp = self.cmd(*query)
    resp = exhaustCursor(self, 'idx', resp)
...
def testMultipleIndexes(self):
    loadDocs(self, idx='idx2', text='goodbye')
    loadDocs(self, idx='idx1', text='hello')
    q1 = ['FT.AGGREGATE', 'idx1', '*', 'LOAD', 1, '@f1', 'WITHCURSOR', 'COUNT', 10 ]
    q2 = q1[::]
    q2[1] = 'idx2'
    r1 = exhaustCursor(self, 'idx1', self.cmd( * q1))
    r2 = exhaustCursor(self, 'idx2', self.cmd( * q2))
...
def testCapacities(self):
    ...
    loadDocs(self, idx='idx1')
    loadDocs(self, idx='idx2')
    q1 = ['FT.AGGREGATE', 'idx1', '*', 'LOAD', '1', '@f1', 'WITHCURSOR', 'COUNT', 10]
    q2 = q1[::]
    q2[1] = 'idx2'
    ...
    self.assertEqual(0, getCursorStats(self, 'idx1')['index_total'])
