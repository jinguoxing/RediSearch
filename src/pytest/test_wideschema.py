...
def testWideSchema(self):
    ...
    for _ in self.reloading_iterator():
        for i in range(FIELDS):
            ...
            res = self.cmd('ft.search', 'idx', '@field_%d:token_%d' % (i, i), 'NOCONTENT')
            ...
            res = self.cmd('ft.search', 'idx', 'hello @field_%d:token_%d' % (i, i), 'NOCONTENT')
            self.assertEqual(res[0], N)

        res = self.cmd('ft.search', 'idx', ' '.join(
