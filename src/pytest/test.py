...
def testAdd(self):
    ...
    for _ in r.retry_with_rdb_reload():
        prefix = 'ft'
        self.assertExists(prefix + ':idx/hello')
        self.assertExists(prefix + ':idx/world')
        self.assertExists(prefix + ':idx/lorem')
...
def testDelete(self):
    ...
    for i in range(100):
        ...
        if i % 2 == 0:
            ...
            self.Expect('ft.get', 'idx', 'doc%d' % i).NotRaiseError()
        res = r.execute_command(
...
def testCreationOptions(self):
    ...
    for x in range(1, 5):
        for combo in combinations(('NOOFSETS', 'NOFREQS', 'NOFIELDS', ''), x):
            _test_create_options_real(self, *combo)
...
def testIssue422(self):
    self.cmd('ft.create', 'myIdx', 'schema',
             'title', 'TEXT', 'WEIGHT', '5.0',
             'body', 'TEXT',
             'url', 'TEXT')
    self.cmd('ft.add', 'myIdx', 'doc1', '1.0', 'FIELDS', 'title', 'hello world', 'body', 'lorem ipsum', 'url', 'www.google.com')
    rv = self.cmd('ft.search', 'myIdx', 'no-as')
